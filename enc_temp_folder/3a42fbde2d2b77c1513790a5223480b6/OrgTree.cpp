...
//For a tree of n employees, the space requirement is 3n pointers.
	...
	bool OrgTree::read(string filename) {
		...
		while (inFile.eof() == false) {
			...
			//If line isn't a close paren
				...
				//Get rid of space before name carried over
		...
		//Check that when exiting, the currentNode is TreeNullPTR; if not, then return false (not enough end parens)
		if (currentNode != TREENULLPTR) {
			return false;
		}
	...
	bool OrgTree::fire(string formerTitle) {
		//Declare variables
		TREENODEPTR fireNode;
		TREENODEPTR childNode;
		TREENODEPTR parentNode;
		TREENODEPTR firesLeftSibling;
	...
	//Best and worst case runtime: Theta(n) (must traverse every node to delete it)
	void OrgTree::recursiveDelete(TREENODEPTR deleteNode) {
		//If the node has a left child, recurse to it.
		if (deleteNode->leftmostChild != TREENULLPTR) {
			//Go to the leftChild of the node
			recursiveDelete(deleteNode->leftmostChild);
		}

		//If the node has a left child, recurse to it.
		if (deleteNode->rightSibling != TREENULLPTR) {
			//Go to the rightSibling of the node
			recursiveDelete(deleteNode->rightSibling);
		}

		//Delete all references to and from the node and node data, then delete node
		deleteNode->leftmostChild = TREENULLPTR;
		deleteNode->name.clear();
		deleteNode->parent = TREENULLPTR;
		deleteNode->rightSibling = TREENULLPTR;
		deleteNode->title.clear();
		delete deleteNode;
